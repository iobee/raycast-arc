...
import { searchTabsPreferences } from "./preferences";
...
function SearchTabs(props: LaunchProps) {
  ...
  const { data, isLoading, mutate } = useCachedPromise(getTabsWithSpaces);
  ...
  const filterTabs = (tabs: EnhancedTab[] | undefined, searchText: string, tabsFilter: string) => {
    ...
    if (searchText.trim() === "") {
      ...
      filteredTabs = tabs
        ...
        .filter((tab) => {
          if (!tab?.space?.title) return true;
          return searchTabsPreferences.excludeSpaces? !searchTabsPreferences.excludeSpaces.toLowerCase().includes(tab.space.title.toLowerCase())
            : true;
        });
    } else {
